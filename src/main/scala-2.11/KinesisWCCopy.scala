...
/**
  ...
  */
object KinesisWCCopy extends Logging {
  def main(args: Array[String]) {
    ...
    // Convert each line of Array[Byte] to String, and split into one line(\r\n)
    val words = unionStreams.flatMap(byteArray => new String(byteArray).split("\\r?\\n"))
...
/**
  ...
  */
private object StreamingExamplesCopy extends Logging {
  // Set reasonable logging levels for streaming if the user has not configured log4j.
  def setStreamingLogLevels() {
    System.out.println("sdfkjhgk")
